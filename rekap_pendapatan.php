...
<?php
    ...
    $sql = " SELECT A.tempat,A.amount,A.feenonadmin,A.adminjastel,A.tanggal as tanggal,( Case when A.amount is NULL then '0' else A.amount END)+(Case when A.adminjastel is NULL then '0' else A.adminjastel END) as jumlahjastel FROM (SELECT lokasi.tempat_pisah as tempat, lokasi.loket as loket,lokasi.lokasi as lokasi,sopp.tanggal as tanggal,  sum(sopp.amount) as amount, CASE when lokasi.loket='lembong1' then 0 when lokasi.loket='lembong2' then 0 when lokasi.loket='lembong3' then 0  END as feenonadmin,sum(sopp.surcharge) as adminjastel from lokasi left join sopp on lokasi.loket=sopp.user AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2' group by lokasi.tempat_pisah) AS A";
    
    $sql2 = "SELECT lokasi.tempat_pisah as tempat, lokasi.loket loket,pln.tanggal as tanggal, sum(pln.total_kopeg) as penpln,sum(pln.bill) as bill, ceiling(pln.fee_admin/pln.bill) as bagi, sum(pln.total_kopeg)-sum(case when pln.fee_admin/pln.bill=2750 then 2000*pln.bill when pln.fee_admin/pln.bill=3000 then 2000*pln.bill when pln.fee_admin/pln.bill=5000 then 3300*pln.bill end) as hasil, sum(case when pln.fee_admin/pln.bill=2750 then 2000*pln.bill when pln.fee_admin/pln.bill=3000 then 2000*pln.bill when pln.fee_admin/pln.bill=5000 then 3300*pln.bill end) as hasil2 from lokasi left join pln on lokasi.loket=pln.loket  AND pln.tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  group by lokasi.tempat_pisah";
    
    $sql3 = "SELECT lokasi.lokasi AS pdlokasi,lokasi.tempat_pisah AS tempat, pdam.tanggal AS pdtanggal, lokasi.loket AS pdloket, SUM(pdam.bill) AS pdbill, SUM(pdam.total_kopeg)-SUM(fee_pdam.`fee_mitra`) AS penpdam, SUM(fee_pdam.`fee_mitra`) AS feepdam FROM lokasi LEFT JOIN pdam ON lokasi.loket = pdam.loket AND 
    pdam.tanggal BETWEEN '$tampil_date' AND '$tampil_date2' LEFT JOIN fee_pdam ON pdam.nama_area=fee_pdam.pdam   GROUP BY lokasi.tempat_pisah";

    $sql4 = "SELECT lokasi.tempat_pisah as tempat,lokasi.loket, voucher.tanggal as tanggal,sum(voucher.total_kopeg) as penvoucher, sum(voucher.fee_ca) as feevoucher from lokasi left join voucher on lokasi.loket=voucher.user AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  group by lokasi.tempat_pisah";
    
    $sql5 = "SELECT lokasi.tempat_pisah as tempat,lokasi.loket, voucher_smart.tanggal as tanggal,sum(voucher_smart.total_kopeg) as penvouchersmart, sum(voucher_smart.fee_ca) as feevouchersmart from lokasi left join voucher_smart on lokasi.loket=voucher_smart.user AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  group by lokasi.tempat_pisah";
    
    $sql6 = "SELECT lokasi.tempat_pisah as tempat,lokasi.loket, voucher_tri.tanggal as tanggal,sum(voucher_tri.total_kopeg) as penvouchertri, sum(voucher_tri.fee_ca) as feevouchertri from lokasi left join voucher_tri on lokasi.loket=voucher_tri.user AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  group by lokasi.tempat_pisah";

    $sql7 = "SELECT lokasi.tempat_pisah as tempat, sum(indovision.fee_finnet) as fee_indovision, sum(indovision.kewajiban) as pen_indovision from lokasi left join indovision on lokasi.loket=indovision.loket AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2' group by lokasi.tempat_pisah";

    $sql8 = "SELECT lokasi.tempat_pisah as tempat, sum(transvision.surcharge-transvision.fee_finnet) as fee_transvision, sum(transvision.kewajiban) as pen_transvision from lokasi left join transvision on lokasi.loket=transvision.loket AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2' group by lokasi.tempat_pisah";

    $sql9 = "SELECT lokasi.tempat_pisah as tempat, sum(aora.fee_finnet) as fee_aora, sum(aora.kewajiban) as pen_aora from lokasi left join aora on lokasi.loket=aora.loket AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2' group by lokasi.tempat_pisah";

    $sql10 = "SELECT lokasi.tempat_pisah AS tempat, arindo_trx.tanggal AS tanggal, SUM(arindo_trx.total_trx) AS arindo_pen FROM lokasi LEFT JOIN arindo_trx ON lokasi.user = arindo_trx.kode_user AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  GROUP BY lokasi.tempat_pisah";

    $sql11 = "SELECT S.tempat,SUM(R.fee_kopeg) AS fee_arindo FROM lokasi AS S LEFT JOIN fee_arindo AS P ON(S.user=P.kode_user) LEFT JOIN kode_awal AS Q ON (LEFT(id,3)=nomor_awal) LEFT JOIN harga_pulsa AS R ON Q.produk=R.produk AND P.tagihan=R.harga_arindo AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2' GROUP BY S.tempat_pisah";

    $sql12 = "SELECT lokasi.tempat_pisah as tempat, lokasi.loket, rk_input.tanggal as tanggal, rk_input.jumlah as jumlah_rk from lokasi left join rk_input on lokasi.tempat_pisah = rk_input.tempat AND tanggal BETWEEN '$tampil_date' AND '$tampil_date2'  group by lokasi.tempat_pisah";
    ...
    <div class="table-responsive">
        ...
        <thead>
            <tr>
                ...
                <th colspan="4" style="text-align:center;">Jastel</th>
                <th colspan="2" style="text-align:center;">PLN</th>
                <th colspan="2" style="text-align:center;">PDAM</th>
                <th colspan="2" style="text-align:center;">Voucher ALL</th>
                <th colspan="2" style="text-align:center;">Indovision</th>
                <th colspan="2" style="text-align:center;">Transvision</th>
                <th colspan="2" style="text-align:center;">Aora TV</th>
                <th colspan="2" style="text-align:center;">Transaksi Arindo</th>
                <th rowspan="2" style="text-align:center;">Total Transaksi</th>
                <?php
