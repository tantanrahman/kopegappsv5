<?php 
    ...
    $sql =  "SELECT arindo_tempat.user, arindo_tempat.kode_user, sum(pln_trx) as    pln_trx ,sum(telepon_trx) as telepon_trx ,sum(indovision_trx) as indovision_trx, sum(halo_trx) as halo_trx, sum(pulsa_trx) as pulsa_trx, sum(pdam_trx) as pdam_trx, sum(adira_trx) as adira_trx, sum(baf_trx) as baf_trx, sum(fif_trx) as fif_trx, sum(bpjs_trx) as bpjs_trx, sum(plnp_trx) as plnp_trx, sum(wom_trx) as wom_trx, sum(total_trx) as total_trx from arindo_tempat left join arindo_trx on arindo_tempat.kode_user=arindo_trx.kode_user and tanggal between '$tampil_date' and '$tampil_date2' group by user";
?>

<div class="container-fluid mt-3">
    <div class="alert alert-dark text-center">
        <b>Lembar Pendapatan ARINDO
            <?php 
