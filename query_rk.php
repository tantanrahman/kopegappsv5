...
for ($i=0;$i<count($_POST['rk']);$i++){
    ...
    if ($rows == null) {
        ...
        $insert = "INSERT INTO `kopeg`.`rk_input` VALUES ('$pecah[0]', '$pecah[1]', $rk)";
...
for ($i=0;$i<count($_POST['arindo_fee']);$i++) {
    ...
    $rows = mysqli_fetch_array($ambildata5, MYSQL_ASSOC);
	if ($rows == null){
        $insert = "INSERT INTO `kopeg`.`arindo_input` (tanggal,tempat,fee) VALUES ('$pecah[0]', '$pecah[1]', $fee)";
		$sql=mysqli_query($konek,$insert);
	}else {
...
for ($i=0;$i<count($_POST['arindo_pen']);$i++) {
	...
	if ($rows == null) {
        $insert = "INSERT INTO `kopeg`.`arindo_input` (tanggal,tempat,pen) VALUES ('$pecah[0]', '$pecah[1]', $pen)";
		$sql=mysqli_query($konek,$insert);
	}else{
...
$data = mysql_fetch_array($eksekusi,MYSQL_ASSOC);
	...
	if($data == null){
        $insert = "INSERT INTO `kopeg`.`pengawasan` VALUES ($transfer1,$transfer2,$transfer3,$transfer4,$biaya,$tarik_tunai,$jasa_giro,$pph,$kel_giro,'$tanggal')";
	}else{
